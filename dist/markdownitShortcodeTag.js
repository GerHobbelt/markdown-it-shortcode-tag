...
function shortcode_plugin(md, shortcodes, options) {
  ...
  const interpolator = options.interpolator || function (expr, env) {
    return env[expr];
  };
  ...
  const reString = '(\'[^\']*\'|"[^"]*")';
  const reExpr = '(#\\{[^}]*\\})';
  const reAttrs = new RegExp('\\s+' + reName + '(?:\\s*=\\s*(?:' + reNumber + '|' + reString + '|' + reExpr + '))?', 'g');
  ...
  const fallback = function fallback(tokens, idx, options, env, self) {
...
export default shortcode_plugin;
//# sourceMappingURL=markdownItShortcodeTag.modern.js.map
